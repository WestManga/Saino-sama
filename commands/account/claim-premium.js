  ...
  run: async (client, message) => {
    ...
    if (point === 0)
      return message.channel
        .send(
          `Kamu tidak punya point yang cukup untuk claim Supporter!\nDibutuhkan minimum **5 Point** untuk claim Bronze Supporter.`
        )
    ...
    if (point > 4 && point < 15) {
      if (data.account.patreon === "Bronze")
        return message.channel
          .send(`You already Bronze Supporter!`)
    ...
    } else if (point > 24 && point < 49) {
      if (data.account.patreon === "Silver")
        return message.channel
          .send(`You already Silver Supporter!`)
          .then((d) => d.delete({ timeout: 20000 }));
      ...
      data.save();
      message.channel.send({ embed: e });
    } else if (point > 49 && point < 99) {
      if (data.account.patreon === "Gold")
        return message.channel
          .send(`You already Gold Supporter!`)
          .then((d) => d.delete({ timeout: 20000 }));
      ...
      if (data.account.patreon === "Platinum")
        return message.channel
          .send(`You already Platinum Supporter!`)
          .then((d) => d.delete({ timeout: 20000 }));
    ...
    let notifembed = new MessageEmbed()
      ...
      .setDescription(
        `Dear <@338418945620967434> seseorang telah menjadi supporter baru!`
      )
      .addField(
        "Informasi User",
