...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (mention) {
      embed.setAuthor(
        mention.tag,
        mention.displayAvatarURL({ size: 4096, dynamic: true })
      ); //author embed
      embed.setDescription(
        `[Avatar URL](${mention.displayAvatarURL({
          size: 4096,
          dynamic: true,
        })})`
    ...
    if (userID) {
      embed.setAuthor(
        userID.user.tag,
        userID.user.displayAvatarURL({ size: 4096, dynamic: true })
      );
      embed.setDescription(
    ...
    if (find) {
      embed.setAuthor(
        find.user.tag,
        find.user.displayAvatarURL({ size: 4096, dynamic: true })
      );
      embed.setDescription(
