    ...
    getPopular(message) {
        return new Promise(async (fullfill, reject) => {
            try {
                ...
                ? "#ffffff"
                : message.guild.me.displayHexColor;
                let get = await axios.get(`https://westapi.herokuapp.com/api/manga/popular`);
                let data_search = get.data.manga_list;
                if (data_search.length < 1) return message.reply(`Pencarian manga popular di Westmanga tidak ditemukan!`);
    ...
    getPJUpdate(message) {
        return new Promise(async (fullfill, reject) => {
            try {
                ...
                ? "#ffffff"
                : message.guild.me.displayHexColor;
                let get = await axios.get(`https://westapi.herokuapp.com/api/manga/newupdate`);
                let data_search = get.data.manga_list;
                if (data_search.length < 1) return message.reply(`Pencarian manga popular di Westmanga tidak ditemukan!`);
                ...
                let embed = new Discord.MessageEmbed()
                    .setColor(roleColor)
                    .setTitle('Hasil Pencarian')
                    .setDescription(data_search.map((a, i) => `${i + 1}. ${a.title}\`\`\`${a.chapter} | ${a.update_on}\`\`\``).join('\n'))
                let embed_search = await message.channel.send(embed);
                let alert_search = await message.reply('pilih untuk melanjutkan!');
