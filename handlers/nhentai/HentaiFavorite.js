...
class HentaiFavorite {
  ...
  setUserFavoritID(member, book) {
    return new Promise(async (resolve, reject) => {
      try {
        let data = await this.client.hdb.db.run(
  ...
  async favoriteEmbed(msg, id = msg.author.id) {
    try {
      ...
      let info = this.client.hentaiembed.getInfo(res);
  ...
  async getFavoriteEmoji(id, embed, m, msg, member) {
    try {
      ...
      infor.on("collect", async i => {
        ...
        info = this.client.hentaiembed.getInfo(res);

        await this.client.hentaiembed.getInfoEmbed(info.id, msg);
      });
